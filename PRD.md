...
- **Functionality**: Create and join group discussions with multiple friends for collaborative Bible study, send messages to entire group with verse references, react to messages with emojis (thumbs up, heart, praying hands, fire), view group member lists with roles (admin, moderator, member), customize group with name, description, topic, and related passage/reading plan, control privacy settings (public, friends-only, invite-only), invite additional members to existing groups, share verses from Reader directly to multiple groups, view group information and settings, pin important messages, and manage notifications per group
- **Purpose**: Foster collaborative Bible study and community fellowship by enabling multiple believers to discuss scripture together, share insights in group context, build study accountability with peers, and create dedicated spaces for specific topics (books of the Bible, themes, church groups, etc.)
- **Trigger**: Groups tab; tap "Create Group" button; from Reader's verse share dialog, select Groups tab to share verse with group; from friend list, invite to existing group; group notification badge
- **Progression**: Groups tab shows list of joined groups with member counts and recent activity → Tap "Create Group" → Enter group name, description, optional topic → Choose privacy level (public/friends-only/invite-only) → Select friends to invite → Create group → Group thread opens with welcome message → Send messages to discuss passages → Share verse from Reader by tapping Share → Select Groups tab → Choose target groups → Add message → Send to all selected groups → Verse appears in each group thread with formatted reference → Members react with emoji reactions → Tap group header for info → View members list with roles → Tap "Invite Members" to add more friends → Admin can manage settings → Pin important messages to top → Leave group if no longer participating → Unread message badges show total across all groups
- **Success criteria**: Group creation completes in <1s; groups support up to 50 members; messages send instantly to all members; verse sharing works from Reader to multiple groups simultaneously; reactions update in real-time; member invites send notifications; group list shows accurate unread counts; pinned messages remain at top of thread; privacy settings correctly limit visibility and joining; mobile and desktop layouts adapt appropriately; group threads persist message history; search works across group messages; admins can remove members and change settings
