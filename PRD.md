...
### 16. Verse of the Day with Scheduled Notifications
- **Functionality**: Display a daily inspirational verse on the home screen with automatic rotation, featuring sharing options to friends and groups, copy to clipboard, bookmark functionality, like/favorite capability, seamless navigation to read the verse in its full chapter context, and configurable daily notifications at user-selected times to remind users to read their verse of the day
- **Purpose**: Inspire daily spiritual reflection through carefully selected meaningful verses, encourage sharing and discussion of Scripture within the community, provide a simple entry point for daily Bible engagement, and build consistent devotional habits through timely reminders that respect user schedules
- **Trigger**: Home screen loads; verse automatically appears on home view; share button opens sharing dialog; read in context button navigates to Reader; scheduled notifications appear at user-configured times; Settings → Notifications tab to configure times
