...
### 16. Verse of the Day
- **Functionality**: Display a daily inspirational verse on the home screen with automatic rotation, featuring sharing options to friends and groups, copy to clipboard, bookmark functionality, like/favorite capability, and seamless navigation to read the verse in its full chapter context
- **Purpose**: Inspire daily spiritual reflection through carefully selected meaningful verses, encourage sharing and discussion of Scripture within the community, and provide a simple entry point for daily Bible engagement
- **Trigger**: Home screen loads; verse automatically appears on home view; share button opens sharing dialog; read in context button navigates to Reader
- **Progression**: Home view loads → Verse of the Day card displays with today's date → Verse text shown with book, chapter, verse reference and translation → Tap heart icon to like/favorite the verse → Tap Copy to copy formatted verse text to clipboard → Tap Bookmark to save verse with auto-generated label → Tap Share to open sharing dialog → Choose to share with individual friends via messages or post to groups → Share dialog shows friend list and group list with member counts → Tap friend to open message thread with verse pre-filled → Tap group to share verse to group discussion → Tap "Read in Context" button to navigate directly to Reader view at that chapter → New verse automatically loads at midnight each day → Previously liked verses tracked across days
- **Success criteria**: Verse loads within 2 seconds on home screen; verse changes daily at midnight with cache persistence; inspirational verses selected from meaningful passages (John 3:16, Psalm 23, Philippians 4:13, etc.); sharing dialog opens smoothly with friend/group lists populated; copy function works reliably across all platforms; bookmark creates entry with auto-generated descriptive label; like/favorite persists across sessions; "Read in Context" navigates to correct chapter in Reader; verse caches for offline viewing; sharing to friends opens messages view with verse reference formatted; sharing to groups posts formatted verse text; date displays in readable format; translation displayed matches user preference; verse selection rotates through 30+ inspirational passages on annual cycle
